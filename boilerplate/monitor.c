 ...
 *
 * We use a mutex (not a spinlock) because our timer callback calls
 * get_task_mm / mmput which can sleep, and we iterate the list
 * inside that callback. Spinlocks forbid sleeping; mutex allows it.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe allows us to delete entries during
     * iteration without corrupting the list walk.
     */
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        long rss = get_rss_bytes(entry->pid);

        /* Process no longer exists — remove the entry */
        if (rss < 0) {
            list_del(&entry->list);
            kfree(entry);
            continue;
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING "[container_monitor] soft limit > hard limit, rejected\n");
            return -EINVAL;
        }
